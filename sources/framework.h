...
inline mat4 rotate(const float angle, const vec3& v) {
...
class GPUProgram {
    ...
    bool checkShader(const unsigned int shader,
                     std::string message) { // shader ford�t�si hib�k kezel�se
        ...
        if (!result) {
            ...
            glGetShaderInfoLog(shader, infoLogLength, NULL,
                               static_cast<GLchar*>(errorMessage.data()));
    ...
    bool checkLinking(
        const unsigned int program) { // shader szerkeszt�si hib�k kezel�se
        GLint infoLogLength = 0, result = 0;
    ...
    int getLocation(
        const std::string& name) { // uniform v�ltoz� c�m�nek lek�rdez�se
        const int location =
    ...
    std::string shaderType2string(const GLenum shadeType) {
        switch (shadeType) {
  ...
  public:
    ...
    bool addShader(const GLenum shaderType, const fs::path& _fileName) {
    ...
    void setUniform(const int i, const std::string& name) {
        const int location = getLocation(name);
        if (location >= 0)
            glUniform1i(location, i);
    }

    void setUniform(const float f, const std::string& name) {
        const int location = getLocation(name);
        if (location >= 0)
...
class Geometry {
  ...
  public:
    Geometry() {
        ...
        const int nf = min(static_cast<int>(sizeof(T) / sizeof(float)), 4);
    ...
    void updateGPU() { // CPU -> GPU
        ...
        const GLsizeiptr bytes =
            static_cast<GLsizeiptr>(vtx.size()) * sizeof(T);
        const void* data =
            vtx.empty() ? nullptr : vtx.data(); // <-- don't touch [0] if empty
    ...
    void Draw(GPUProgram* prog, const int type, const vec3 color) {
        if (vtx.size() > 0) {
            prog->setUniform(color, "color");
            glBindVertexArray(vao);
            glDrawArrays(type, 0, static_cast<int>(vtx.size()));
        }
    }
...
class Texture {
  ...
  public:
#ifdef FILE_OPERATIONS
    Texture(const fs::path pathname, const bool transparent = false,
            const int sampling = GL_LINEAR) {
        if (textureId == 0)
    ...
    Texture(const int width, const int height) {
        ...
        constexpr vec3 yellow(1, 1, 0), blue(0, 0, 1);
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB,
                     GL_FLOAT, &image[0]); // To GPU
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                        GL_NEAREST); // sampling
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    }

    Texture(const int width, const int height, std::vector<vec3>& image) {
        glGenTextures(1, &textureId);            // azonos�t� gener�l�sa
        glBindTexture(GL_TEXTURE_2D, textureId); // ez az akt�v innent�l
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB,
                     GL_FLOAT, &image[0]); // To GPU
